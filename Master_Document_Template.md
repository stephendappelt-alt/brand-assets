...
7. **Apply the Logo Contrast Rule** (see Section 3.1) to auto-select the correct logo variant
8. Validate and render a preview
9. Ship
...
### 3.1 Logo Contrast Rule (HARD RULE)

**This rule is universal and non-negotiable:**

> **Letter color MUST contrast against the background color.**
> - White-letter logos → go on colored or dark backgrounds ONLY
> - Navy-letter / dark-letter logos → go on white or light backgrounds ONLY

**Never place a white-letter logo on a white background.** It becomes invisible or ghosted, which is the exact failure that happened on the first BGP board update test.

**Auto-selection logic at build time:**
```
IF component_background == WHITE or LIGHT_GRAY:
    USE [brand]-logo-*-navy-letters.png (or equivalent dark-letter variant)
    Fallback: USE the color/dark variant from Registry
ELIF component_background == BRAND_PRIMARY (dark) or PHOTO:
    USE [brand]-logo-*-white-letters.png (or equivalent white/knockout variant)
ELSE:
    Flag to Stephen and ask before proceeding
```

**Filename convention to support this rule:**
- `[brand]-logo-wide-navy-letters.png` — horizontal, dark letters (for light bg)
- `[brand]-logo-wide-white-letters.png` — horizontal, white letters (for dark bg)
- `[brand]-logo-square-navy-letters.png` — square, dark letters (for light bg)
- `[brand]-logo-square-white-letters.png` — square, white letters (for dark bg)
- Simpler names (`bps-logo-wide.png`, `ags-logo.png`) are acceptable when there's only one variant per context — the Registry specifies which to use.
...
**Background color:** WHITE (default) or BRAND_PRIMARY (for heavy-brand headers).

**Logo variant selection (REQUIRED — apply Section 3.1 Logo Contrast Rule):**
- IF Header Band background is WHITE → use the **navy/dark-letter wide variant** (`[brand]-logo-wide-navy-letters.png` or brand's default color wide logo)
- IF Header Band background is BRAND_PRIMARY or dark → use the **white-letter wide variant** (`[brand]-logo-wide-white-letters.png`)
- If the required variant is not present in the repo, flag the gap to Stephen BEFORE building

**Specs:**
- Logo: Render at size from Registry (e.g. BPS = 220x57, BGP = 195x50)
...
**When to use:** One-pagers, proposals, board updates. 4 stats minimum.
...
**Logo use inside this component:** If a logo is placed inside a Call-Out Band, it MUST be the white-letter variant (apply Section 3.1 Logo Contrast Rule).
...
- No white-knockout logo yet → on any component requiring dark background logo, fall back to regular logo on white chip, or skip logo.
...
- **Logo handling:** BGP has navy-letters and white-letters variants. ALWAYS apply Section 3.1 Logo Contrast Rule. Current gap: no wide-navy-letters variant exists — use the square-navy-letters version at scaled-down horizontal crop, or flag to Stephen.
...
- [ ] Logo is the correct variant for background (Section 3.1 Logo Contrast Rule applied)
...
- [ ] Logo is visible and readable against its background (sanity check)
...
3. Ensure the brand has both white-letters and navy/dark-letters variants for each logo shape (wide, square) to comply with Section 3.1 Logo Contrast Rule
4. No changes needed to this Master Template — it auto-adapts via brand variables
...
- ❌ **Never place a white-letter logo on a white or light background** (Section 3.1 Logo Contrast Rule)
- ❌ **Never place a navy-letter / dark-letter logo on a dark or photo background** (Section 3.1 Logo Contrast Rule)
- ❌ Never use Unicode bullet characters in raw text (•, ‣, ▪) — use LevelFormat.BULLET
...
- ❌ Never use filenames with spaces — always use hyphens (e.g. `bgp-logo-wide-navy-letters.png` NOT `bgp-logo-wide navy letters.png`)
...
|---|---|---|
| 2026-04-18 | v1.1 | Added Section 3.1 Logo Contrast Rule (white letters on dark bg, navy letters on light bg). Added auto-variant-selection logic to Header Band (5.1). Updated Section 10 with logo contrast hard rules and filename space rule. Updated QC checklist with visibility check. Updated Section 9 onboarding to require both letter-color variants. |
